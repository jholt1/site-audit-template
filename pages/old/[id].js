import Head from 'next/head';
import Link from 'next/link';
import styles from '../../styles/Report.module.css';

export async function getServerSideProps({ res, params, query }) {
  const url = query.d ? `${process.env.URL_PREFIX}/${params.id}_${query.d}.html` : `${process.env.URL_PREFIX}/${params.id}.html`;
  const data = await fetch(url);

  if (data.status !== 200) {
    res.setHeader('location', '/');
    res.statusCode = 302;
    res.end();
    return { props: {} };
  }

  const report = await (data).text();

  return {
    props: {
      data: {
        report
      }
    }
  }
};

export default function OldAudit({ data }) {
  return (
    <>
      <Head>
        <title>Lighthouse Audit</title>
        <link rel="icon" href="data:image/png;base64," />
      </Head>
      <header className={styles.reportDetails}>
        <Link href="/" passHref><a>&larr; Back</a></Link>
      </header>
      <div dangerouslySetInnerHTML={{ __html: data.report }} />
    </>
  );
};